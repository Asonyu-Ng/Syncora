...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="bg-slate-50 font-['Inter'] text-slate-900 antialiased">
        @if ($variant === 'split')
            <div class="relative min-h-screen overflow-hidden bg-[#f5f7fb]">
                <div class="absolute inset-0 bg-[radial-gradient(circle_at_top_left,_rgba(99,102,241,0.12),_transparent_30%),radial-gradient(circle_at_bottom_right,_rgba(14,165,233,0.12),_transparent_28%),linear-gradient(180deg,_#f8fafc_0%,_#eff4ff_100%)]"></div>
                <div class="absolute inset-y-0 left-0 hidden w-1/2 bg-[radial-gradient(circle_at_top,_rgba(255,255,255,0.28),_transparent_42%)] lg:block"></div>

                <main class="relative flex min-h-screen items-center justify-center px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
                    <div class="w-full max-w-7xl overflow-hidden rounded-[2rem] border border-slate-200/70 bg-white shadow-[0_45px_120px_-48px_rgba(15,23,42,0.35)]">
                        <div class="grid min-h-[720px] lg:grid-cols-[1.08fr_minmax(420px,0.92fr)]">
                            <section class="relative overflow-hidden bg-slate-950 px-6 py-8 text-white sm:px-8 sm:py-10 lg:px-10 lg:py-12">
                                <div class="absolute inset-0 bg-[radial-gradient(circle_at_top_left,_rgba(129,140,248,0.42),_transparent_34%),radial-gradient(circle_at_78%_24%,_rgba(56,189,248,0.28),_transparent_26%),linear-gradient(180deg,_rgba(15,23,42,0.92)_0%,_rgba(30,41,59,0.96)_100%)]"></div>
                                <div class="absolute inset-x-8 top-0 h-px bg-gradient-to-r from-white/0 via-white/30 to-white/0"></div>
                                <div class="absolute -left-20 bottom-10 h-52 w-52 rounded-full bg-indigo-400/25 blur-3xl"></div>
                                <div class="absolute right-0 top-16 h-56 w-56 rounded-full bg-cyan-400/20 blur-3xl"></div>
                                ...
                                <div class="relative flex h-full flex-col">
                                    <a href="{{ url('/') }}" class="inline-flex w-fit items-center gap-3 rounded-full border border-white/15 bg-white/10 px-4 py-2 text-left backdrop-blur transition hover:bg-white/14 focus:outline-none focus:ring-4 focus:ring-white/15">
                                        <div class="grid h-11 w-11 place-items-center rounded-full bg-white text-slate-950 shadow-lg shadow-slate-950/20">
                                            <x-application-logo class="h-6 w-6 fill-current text-slate-950" />
                                        </div>
                                        <div>
                                            <div class="text-sm font-semibold tracking-tight text-white">{{ $authBrandName }}</div>
                                            <div class="text-xs text-slate-300">Internship workflow platform</div>
                                        </div>
                                    </a>
                                    ...
                                    <div class="mt-8 grid gap-3 sm:grid-cols-3">
                                        <div class="rounded-[1.5rem] border border-white/10 bg-white/8 px-4 py-4 backdrop-blur">
                                            <div class="text-xs font-semibold uppercase tracking-[0.18em] text-slate-300">Unified Access</div>
                                            <div class="mt-2 text-sm text-slate-100">One secure entry point for students, supervisors, and company teams.</div>
                                        </div>
                                        <div class="rounded-[1.5rem] border border-white/10 bg-white/8 px-4 py-4 backdrop-blur">
                                            <div class="text-xs font-semibold uppercase tracking-[0.18em] text-slate-300">Live Workflow</div>
                                            <div class="mt-2 text-sm text-slate-100">Stay aligned on approvals, task progress, reports, and internship milestones.</div>
                                        </div>
                                        <div class="rounded-[1.5rem] border border-white/10 bg-white/8 px-4 py-4 backdrop-blur">
                                            <div class="text-xs font-semibold uppercase tracking-[0.18em] text-slate-300">Trusted Routing</div>
                                            <div class="mt-2 text-sm text-slate-100">Sign in and land in the right dashboard with the same protected auth flow.</div>
                                        </div>
                                    </div>
                                </div>
                            </section>
