...
<div x-data="{ open: false }" @keydown.escape.window="open = false" class="relative">
    ...
    >
        <div class="hidden sm:block text-right">
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400">{{ Str::limit($email, 20) }}</p>
        </div>
        <x-avatar :name="$name" :email="$email" size="md" />
        <svg class="w-4 h-4 text-gray-400 dark:text-gray-500 transition-transform duration-200" :class="open ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div
        ...
        class="absolute right-0 mt-2 w-60 bg-white dark:bg-gray-900 rounded-xl shadow-xl ring-1 ring-black ring-opacity-5 py-2 z-50"
        style="display: none;"
    >
        ...
        <a
            ...
            class="flex items-center px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
        >
        ...
        @if(Route::has('logout'))
            <button
                ...
                class="w-full text-left flex items-center px-4 py-2 text-sm text-red-600 hover:bg-red-50 dark:hover:bg-red-950/20 transition-colors"
            >
                <svg class="w-4 h-4 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
